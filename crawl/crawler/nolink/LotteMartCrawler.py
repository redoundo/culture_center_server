from crawler.utils.utils import adjust_white_space
...
class LotteMartCrawler(NoLinkCrawler):
    ...
    def search_option_setting(self):
        ...
        if not self.page.is_visible("#frm > div > ul > li:nth-child(1) > div > div.branch_find-wrap.branch_selc-wrap\
            ...
            pass
        region_locators = self.page.locator(
            "#frm > div > ul > li:nth-child(1) > div > div.branch_find-wrap.branch_selc-wrap > div > ul > li").all()
        for region_locator in region_locators:
            region_name: str = region_locator.locator("div.ft-accor-title > a").inner_text()
        ...
        if not self.page.is_visible(f"#frm > div > ul > li:nth-child(1) > div > div.branch_find-wrap.branch_selc-wrap \
                > div > div:nth-child({region_index + 2}) > ul > li:nth-child(1) > a", timeout=6000):
            self.page.click('#chooseStore', delay=200, timeout=4000)  # 지점 선택 버튼 다시 클릭
        ...
        branch_locators = self.page.locator(f"#frm > div > ul > li:nth-child(1) > div > div.branch_find-wrap.branch_selc-wrap > div > div:nth-child({region_index + 1}) > ul > li").all()
        for branch_locator in branch_locators:
            branch_name: str = branch_locator.locator("a").inner_text()
