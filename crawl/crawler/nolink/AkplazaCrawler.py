...
class AkplazaCrawler(NoLinkCrawler):
    ...
    def search_option_setting(self) -> None:
        ...
        branch_locators = self.page.locator(f"#header > div.myaca-wrap.active > div.aca-wr > div > div > div > div > ul > li").all()
        for branch_locator in branch_locators:
            branch_name: str = branch_locator.inner_text()
        ...
        self.page.click(f"#header > div.myaca-wrap.active > div.aca-wr > div > div >\
         div > div > ul > li:nth-child({branch_index + 1})", timeout=3000, delay=20)
