...
class LotteMartCrawler(NoLinkCrawler):
    ...
    def check_lecture_total(self):
        self.page.click("#accepttotcnt > a", delay=20, timeout=50000)
        time.sleep(10)
        raw_total: str = self.page.inner_text("#accepttotcnt > a", timeout=50000)
        ...
        for locator in locators:
            ...
            if param is None:
                param = self.page.get_attribute(f"#course_data > tr:nth-child({locators.index(locator)}) >\
                         td.align-l.dis-first > div.info-txt > a", "onclick", timeout=60000)
            else:
                pass
    ...
    def extract_lecture_info(self, url: str, page: Page, info: dict):
        ...
        # branch: str = (self.page.inner_text("#contents > div.lct_tit-area.mt30 > span.lct-branch")
        #                .replace("[", "").replace("]", ""))
        ...
        if len(prices) >= 1:
            ...
            lecture.price = 0  # TODO : 반드시 필요한 내용이 없으면 에러가 나거나 조취를 취해야 하는데 어떻게 해야 햐는지 모르겠다.
