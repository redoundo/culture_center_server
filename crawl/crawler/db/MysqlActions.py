...
class MysqlActions:
    ...
    def center_info_by_center_name(self, name: str) -> dict[str, BranchInfoForSetting]:

        cursor: connection.MySQLCursor = self.connection.cursor()
        branch_infos: dict[str, BranchInfoForSetting] = dict()
        try:
            sql: str = f"SELECT B.branchId AS branchId, B.branchName AS branchName, C.centerUrl AS centerUrl, C.centerId AS centerId FROM branches AS B LEFT JOIN centers AS C ON C.centerName='{name}' WHERE B.centerIdOfBranch = C.centerId;"
            cursor.execute(sql)
            for branchId, branchName, centerUrl, centerId in cursor.fetchall():
                branch_info: BranchInfoForSetting = BranchInfoForSetting(branch_id=branchId, branch_name=branchName,
                                                                         center_id=centerId, center_name=name,
                                                                         url=centerUrl)
                branch_infos[branchName] = branch_info
        except Exception as e:
            print(e)
        finally:
            self.connection.commit()
    ...
    def add_or_delete_branches(self, method: str, branch_id: int = None, branch_info: BranchesTableInfo = None):
        cursor: connection.MySQLCursor = self.connection.cursor()
        if branch_id is None and branch_info is None:
            return
        try:
            if method == "delete":
                sql: str = f"DELETE FROM branches WHERE branchId={branch_id};"
            else:
                sql: str = f"INSERT INTO branches(branchName, branchAddress, branchState, branchCity, branchTown,\
                 longitude, latitude) VALUES('{branch_info.branchName}', '{branch_info.branchAddress}',\
                  '{branch_info.branchState}', '{branch_info.branchCity}', '{branch_info.branchTown}', \
                  {branch_info.longitude}, {branch_info.latitude});"
            cursor.execute(sql)
        except Exception as e:
            print(e)
        finally:
    ...
    def all_branch_names(self, center: str) -> list[str]:
        cursor: connection.MySQLCursor = self.connection.cursor()
        branches: list[str] = []
        try:
            sql: str = f"SELECT B.branchName AS branchName FROM branches AS B LEFT JOIN centers AS C ON C.centerName='{center}' WHERE B.centerIdOfBranch = C.centerId"
            cursor.execute(sql)
            for branchName in cursor.fetchall():
