...
docker container run -d --name autoheal --restart=always -v /var/run/docker.sock:/var/run/docker.sock -e TZ=Asia/Seoul -e AUTOHEAL_CONTAINER_LABEL=autohealLabel -e AUTOHEAL_INTERVAL=5 -e AUTOHEAL_START_PERIOD=0 -e AUTOHEAL_DEFAULT_STOP_TIMEOUT=10 -e DOCKER_SOCK=/var/run/docker.sock -e CURL_TIMEOUT=30 willfarrell/autoheal:latest